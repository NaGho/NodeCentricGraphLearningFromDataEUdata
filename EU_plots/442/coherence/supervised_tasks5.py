...
class LoadCore(object): 
    def __init__(self, preszr_sec, postszr_sec):
...
#         W = np.transpose(W, [2,3,0,1])
        ...
        for idx in idx_array:
            ...
            W_in = np.squeeze(np.mean(W_in[:, aggregate_conv_sizes[band_num-1]:aggregate_conv_sizes[band_num],:,:], axis=1))
...
#         A_val_string_arr = ['optimizedA' if side_adj_mat is None else 'fixedA_invCov' for side_adj_mat in side_adj_mat_arr] 
        ...
        varType_arr = [['diag_repeated', 'diag_repeated']] # Theta_weighting, Theta_graphL
...
#         freq_bands = [[0.1, 4], [4, 8], [8, 13], [13, 30], [30, 50], [70, 100]]
        ...
        load_core = LoadCore(preszr_sec=10, postszr_sec=10000)
                ...
                if(X_test is not None):
                    ...
                    while(not classif_loadbatch.end()):
                        ...
                        coarse_eval_performance(y_true, prob_hat, self.sensitivity_score_target,\
                                                ...
                                                mvAvg_winlen=self.mvAvg_winlen) 
                        if(Windows and (np.any(y_true!=0) or random.random()<0.2)): #or random.random()<0.1
