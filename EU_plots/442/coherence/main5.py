...
def seizure_state_estimation(build_target):
    ...
    targets = [442] # 
    cv_ratio = 0.5
    feature_mode = 'W_graphL_coherence'   #     'X_raw'    'W_raw_correlation' 'W_raw_coherence'   'W_graphL_correlation'    'W_graphL_coherence'    
...
#             try:
                ...
                task_core = TaskCore(data_dir=str(settings['data-dir-FFT']) if 'FFT' in feature_mode or ('coherence' in feature_mode ) else str(settings['data-dir-Raw']),\
                                     ...
                                     target=target, \
                                     cv_ratio=cv_ratio, adj_calc_mode=['invcov'], feature_mode=feature_mode, TrainTest_mode='50%-50%-fine', supervised=False) 
