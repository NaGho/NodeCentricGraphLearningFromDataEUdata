...
#         clipped_grads_and_variables = [(grad, var) for grad, var in self.grads_and_variables] # tf.clip_by_value(grad) # grad if grad is not None else tf.zeros_like(var)
...
#         self.evel_threshold = self.classif_core.eval_thresh_func(np.vstack(Eval_thresh), axis=0)
    ...
    #             y_hat = inn_y if y_hat is None else np.concatenate((y_hat, inn_y))
                ...
                if(plotting_flag and (np.any(inn_y_true!=0) or random.random()<0.5)): #
...
def pure_plot_sample(inn_true_y, inn_prob_hat, inn_sel_win_num, counter, num_plots, matlab_load_core, feats, task_core, W_method):
    ...
    if(change_arg.size>0):
        pre_ictal_list = np.arange(np.max((0, change_arg[0]-n_pre_szr*2)), change_arg[0], 1) # np.max((inn_true_y.size,n_pre_szr))
        post_ictal_list = np.arange(change_arg[-1] + 1, change_arg[-1] + n_pre_szr*2 , 1) # np.min((inn_true_y.size, change_arg[-1] + num_plots-change_arg.size-pre_ictal_list.size))
        change_arg = list(pre_ictal_list) + list(change_arg) + list(post_ictal_list)
